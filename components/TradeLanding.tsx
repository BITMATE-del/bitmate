...
export default function TradeLanding({eyebrow,title,description,badge,stats,bullets}:Props){
  return <main className="tradePage">
    <section className="tradeHero"><div className="xtShell tradeHeroGrid">
      ...
      <div className="tradeTerminal"><div className="tradeTerminalHead"><span>{badge}</span><b>BTC / USDT</b><em>SPOT</em></div><div className="tradeTerminalBody"><div className="tradeChart"><div className="tradePrice">77,259.40 <span>-0.02%</span></div><svg viewBox="0 0 600 220" preserveAspectRatio="none"><polyline fill="none" stroke="#b9ff31" strokeWidth="4" points="0,175 45,150 85,160 130,118 170,127 210,92 250,101 295,67 335,84 375,54 425,66 470,30 520,43 600,20"/></svg></div><div className="tradeTicket"><b>Order</b><label>Type</label><div>Market</div><label>Amount</label><div>₩ 300,000</div><button>Place Order</button></div></div></div>
    </div></section>
    ...
    <section className="tradeContent"><div className="xtShell tradeFeatureGrid">
      <div><span className="sectionLabel">BUILT FOR CLARITY</span><h2>A focused trading workspace</h2><p>시장 정보와 주문 기능을 한 화면에서 확인하고 필요한 작업으로 빠르게 이동할 수 있습니다.</p><ul>{bullets.map(x=><li key={x}>{x}</li>)}</ul></div>
