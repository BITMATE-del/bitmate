...
export default function MiningGame(){
  ...
  return <main className="miningPage">
    <section className="miningHero"><div className="xtShell miningHeroGrid">
      <div className="miningIntro">
        ...
        <div className="miningKpis">
          ...
          <div><span>오늘 보상</span><b>{wallet.withUnit(game?.today_reward||0)}</b></div>
          <div><span>누적 보상</span><b>{wallet.withUnit(game?.total_reward||0)}</b></div>
    ...
    <section className="xtShell miningQuickStats">
      ...
      <div><span>오늘 예상 보상</span><b>{wallet.withUnit(activeExpected)}</b></div>
    ...
    <section className="miningControl"><div className="xtShell miningControlGrid">
      <div>
        ...
        <div className="amountGrid">{presets.map(v=><button key={v} className={!custom&&amount===v?'selected':''} onClick={()=>{setCustom('');setAmount(v)}}>{wallet.withUnit(v)}</button>)}</div>
        <input className="miningCustomInput" type="number" min="100" placeholder={`직접 입력 (${wallet.unit})`} value={custom} onChange={e=>setCustom(e.target.value)}/>
      </div>
      <div className="miningQuote">
        <span>현재 선택</span><strong>{wallet.withUnit(selectedAmount)}</strong>
        <dl><div><dt>등급</dt><dd>{tier?.name||'금액을 확인하세요'}</dd></div><div><dt>Mining Power</dt><dd>{tier?`${money(tier.mining_power)} TH/s`:'-'}</dd></div><div><dt>Mining Reward Rate</dt><dd>{tier?pct(currentRate):'-'}</dd></div><div><dt>오늘 예상 Mining Reward</dt><dd>{tier?`${wallet.withUnit(expected)}`:'-'}</dd></div><div><dt>다음 정산 예정</dt><dd>내일 09:05 KST</dd></div></dl>
    ...
    <section className="xtShell tierSection"><div className="sectionTitle"><span>MINER RANK</span><h2>Mining 등급</h2></div><div className="tierGrid">{tiers.map(t=><article className={tier?.id===t.id?'active':''} key={t.id}><span>{t.code}</span><h3>{t.name}</h3><b>{money(t.mining_power)} TH/s</b><p>{wallet.withUnit(t.min_principal)}{t.max_principal?` ~ ${wallet.withUnit(t.max_principal)}`:'+'}</p><small>현재 Rate {pct(rateFor(t.id))}</small></article>)}</div></section>
