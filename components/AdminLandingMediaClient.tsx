...
export default function AdminLandingMediaClient(){
 ...
 return <main style={{minHeight:'calc(100vh - 72px)',background:'#08090a',color:'#f4f6f7',padding:'34px 0 80px'}}><div className="xtShell">
  ...
  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(360px,1fr))',gap:16,marginTop:18}}>{slots.map(slot=><section key={slot.slot_key} style={{...box,padding:18}}>
   ...
   <div style={{display:'flex',gap:8}}><label style={{flex:1,height:42,borderRadius:8,background:'#b9ff31',color:'#0d1108',fontWeight:900,display:'grid',placeItems:'center',cursor:'pointer',opacity:busy===slot.slot_key?0.6:1}}>{busy===slot.slot_key?'처리 중...':'이미지 업로드'}<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" disabled={!!busy} style={{display:'none'}} onChange={e=>{const f=e.target.files?.[0];if(f)upload(slot,f);e.currentTarget.value=''}}/></label><button onClick={()=>reset(slot)} disabled={!!busy||!slot.image_url} style={{height:42,padding:'0 14px',border:'1px solid #34393d',borderRadius:8,background:'#111416',color:'#e4e7e8',fontWeight:800}}>기본값 복원</button></div>
  </section>)}</div>
 </div></main>
}
