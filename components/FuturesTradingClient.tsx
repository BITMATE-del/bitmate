...
export default function FuturesTradingClient(){
  ...
  const totalValue=Math.max(0,referencePrice*num(quantity));
  ...
  return <main className={s.page}>
    ...
    <section className={s.terminal}>
      ...
      <section className={`${s.panel} ${s.orderPanel}`}>
        ...
        <label className={s.inputRow}><span>Amount</span><input value={quantity} onChange={e=>setQuantity(e.target.value)} inputMode="decimal"/><em>{baseAsset}</em></label>
        <div className={s.ratios}>{[.25,.5,.75,1].map(r=><button key={r} onClick={()=>setRatio(r)}>{Math.round(r*100)}%</button>)}</div>
        <div className={s.inputRow}><span>Total</span><b className={s.readonlyValue}>{priceFmt(totalValue,2)}</b><em>USDT</em></div>
