...
export default function FuturesTradingClient(){
  ...
  const setRatio=(ratio:number)=>{if(!snapshot?.account)return;const ref=orderType==='LIMIT'?num(price):num(ask||lastPrice);if(ref<=0)return;const q=(num(snapshot.account.available_balance)*ratio*leverage/ref);const scale=Math.pow(10,qtyDigits);setQuantity(String(Math.max(0,Math.floor(q*scale)/scale)))};

  async function submit(side:'BUY'|'SELL'){
    setMessage('');if(!loggedIn){setMessage('로그인 후 선물거래를 이용할 수 있습니다.');return}if(!settingsEnabled){setMessage('현재 선물 신규 거래가 관리자 설정에서 비활성화되어 있습니다.');return}if(!engineMarket){setMessage('이 종목은 실시간 시세/차트는 지원하지만 아직 BITMATE Futures 주문 엔진에는 등록되지 않았습니다.');return}if(!engineReady){setMessage('현재 이 종목은 주문할 수 없는 상태입니다.');return}const q=Number(quantity);if(!Number.isFinite(q)||q<=0){setMessage('주문 수량을 확인하세요.');return}
    ...
    <section className={s.terminal}>
      ...
      <section className={`${s.panel} ${s.orderPanel}`}>
        <div className={s.orderContent}>
          ...
          <div className={s.orderTop}><button className={marginMode==='CROSS'?s.selectedPill:''} onClick={()=>setMarginMode('CROSS')}>Cross</button><button className={marginMode==='ISOLATED'?s.selectedPill:''} onClick={()=>setMarginMode('ISOLATED')}>Isolated</button><label><input type="number" min={1} max={engineMarket?.max_leverage||100} value={leverage} onChange={e=>setLeverage(Math.max(1,Math.min(engineMarket?.max_leverage||100,Number(e.target.value)||1)))}/><span>x</span></label></div>
          <div className={s.orderTabs}><button className={orderType==='MARKET'?s.activeTab:''} onClick={()=>setOrderType('MARKET')}>Market</button><button className={orderType==='LIMIT'?s.activeTab:''} onClick={()=>setOrderType('LIMIT')}>Limit</button><button className={orderType==='TRIGGER'?s.activeTab:''} onClick={()=>setOrderType('TRIGGER')}>Trigger</button><button className={orderType==='TRAILING_STOP'?s.activeTab:''} onClick={()=>setOrderType('TRAILING_STOP')}>Trailing</button></div>
          ...
          <label className={s.inputRow}><span>Quantity</span><input value={quantity} onChange={e=>setQuantity(e.target.value)} inputMode="decimal"/><em>{baseAsset}</em></label>
        ...
        <div className={s.orderFooter}>
          ...
          <div className={s.tradeButtons}><button disabled={busy||!canTrade} className={s.longBtn} onClick={()=>submit('BUY')}>Long</button><button disabled={busy||!canTrade} className={s.shortBtn} onClick={()=>submit('SELL')}>Short</button></div>
