...
export default function AICoreClient(){
  ...
  const loadUser=useCallback(async()=>{
    ...
    if(!user){setCore({});return;}
    const {data:c}=await supabase.rpc('get_my_ai_core_state');
    setCore((c||{}) as CoreState);
    if(c?.session_id){
  ...
  const selectedAmount=custom?wallet.fromDisplay(Number(custom)):amount;
  ...
  async function start(){
    ...
    setConfirming(false);
    if(error){setNotice(error.message.includes('DEMO_USDT_BALANCE_NOT_FOUND')?'USDT 잔액이 없습니다. 잔액을 확인한 뒤 다시 시도해주세요.':error.message.includes('INSUFFICIENT')?'사용 가능 USDT 잔액이 부족합니다.':error.message);return;}
    setNotice('AI 운용을 시작했습니다.'); await loadUser();
  }
  ...
  async function stop(){if(!core.session_id)return;const {error}=await supabase.rpc('stop_demo_ai_session',{p_session_id:core.session_id,p_stop_mode:'CLOSE_WHEN_READY'});if(error)setNotice(error.message);else{setNotice('중지 요청이 접수되었습니다. 진행 중인 주문과 포지션이 정리되면 운용이 종료됩니다.');await loadUser();}}
