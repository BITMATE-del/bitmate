...
export default function ExchangeHeader(){
  ...
  return <header className="xtHeader"><div className="xtShell xtHeaderInner">
    ...
    <nav className={mobileOpen?'mobileOpen':''} style={{flexWrap:'nowrap'}}>
      ...
      <div className="navDropdown" style={desktopMenuStyle} onMouseEnter={()=>setEtfOpen(true)} onMouseLeave={()=>setEtfOpen(false)}>
        ...
        <div className={etfOpen?'tradeDropdown open':'tradeDropdown'}>
          <Link className="tradeMenuItem" href="/crypto-etf" onClick={closeMenus}><span className="tradeIcon">◈</span><span><b>BITMATE INDEX</b><small>여러 디지털자산을 정해진 비중으로 나눠 담는 바스켓형 분산투자</small></span><em>›</em></Link>
          <Link className="tradeMenuItem" href="/crypto-etf/portfolio" onClick={closeMenus}><span className="tradeIcon">◫</span><span><b>My INDEX</b><small>보유 Basket, 평가손익, 구성자산과 매도 관리</small></span><em>›</em></Link>
        </div>
      </div>
