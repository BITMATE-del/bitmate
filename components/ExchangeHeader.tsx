...
export default function ExchangeHeader(){
  ...
  return <header className="xtHeader"><div className="xtShell xtHeaderInner" style={{width:'100%',maxWidth:'none',margin:0,padding:'0 24px',gap:'22px',boxSizing:'border-box'}}>
    ...
    <nav className={mobileOpen?'mobileOpen':''} style={{flex:'0 1 auto',flexWrap:'nowrap',minWidth:0}}>
      <Link style={desktopMenuStyle} href="/cfd" onClick={closeMenus}>CFD</Link>

      <div className="navDropdown" onMouseEnter={()=>setAiOpen(true)} onMouseLeave={()=>setAiOpen(false)}>
        <button style={desktopMenuStyle} className={aiOpen?'navDropButton active':'navDropButton'} onClick={()=>setAiOpen(v=>!v)} aria-expanded={aiOpen}>AI <span>⌄</span></button>
      ...
      <div className="navDropdown" onMouseEnter={()=>setEtfOpen(true)} onMouseLeave={()=>setEtfOpen(false)}>
        <button style={desktopMenuStyle} className={etfOpen?'navDropButton active':'navDropButton'} onClick={()=>setEtfOpen(v=>!v)} aria-expanded={etfOpen}>ETF <span>⌄</span></button>
      ...
      <div className="navDropdown" onMouseEnter={()=>setCopyOpen(true)} onMouseLeave={()=>setCopyOpen(false)}>
        <button style={desktopMenuStyle} className={copyOpen?'navDropButton active':'navDropButton'} onClick={()=>setCopyOpen(v=>!v)} aria-expanded={copyOpen}>Copy <span>⌄</span></button>
      ...
      <div className="navDropdown" onMouseEnter={()=>setMiningOpen(true)} onMouseLeave={()=>setMiningOpen(false)}>
        <button style={desktopMenuStyle} className={miningOpen?'navDropButton active':'navDropButton'} onClick={()=>setMiningOpen(v=>!v)} aria-expanded={miningOpen}>Min <span>⌄</span></button>
      ...
      <Link style={desktopMenuStyle} href="/#markets" onClick={closeMenus}>Market</Link>
