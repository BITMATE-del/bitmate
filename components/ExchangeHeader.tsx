...
const itemStyle={gridTemplateColumns:'28px minmax(0,1fr) 18px',width:'100%',minWidth:0} as const;
...
type QuickLink={href:string;icon:UiIconName;label:string;sub:string};

const baseQuickLinks:QuickLink[]=[
 {href:'/ai-core',icon:'bot',label:'AI Trading',sub:'AI 자동운용'},
 {href:'/copy-trading',icon:'copy',label:'Copy Trading',sub:'트레이더 카피'},
 {href:'/cfd',icon:'margin',label:'CFD Margin',sub:'레버리지 거래'},
 {href:'/crypto-etf',icon:'etf',label:'Crypto ETF',sub:'BITMATE INDEX'},
 {href:'/mining',icon:'mining',label:'Mining',sub:'Digital Mining'},
 {href:'/crypto-loan',icon:'loan',label:'Crypto Loan',sub:'암호화폐 담보 대출'},
 {href:'/#markets',icon:'market',label:'Markets',sub:'실시간 시장'},
];
...
const iconBoxStyle={width:28,height:28,borderRadius:8,display:'grid',placeItems:'center',background:'rgba(185,255,49,.08)',color:'#b9ff31',flexShrink:0} as const;
...
export default function ExchangeHeader(){
  ...
  const quickLinks:QuickLink[]=isLoggedIn?[...baseQuickLinks,{href:'/more/btmt-membership',icon:'membership',label:'BTMT Membership',sub:'BTMT 스테이킹 멤버십'}]:baseQuickLinks;
  ...
  return <header className="xtHeader"><div className="xtShell xtHeaderInner" style={{width:'100%',maxWidth:'none',margin:0,padding:'0 24px',gap:'22px',boxSizing:'border-box'}}>
    ...
    <div className="xtHeaderTools" style={{marginLeft:'auto',flexShrink:0,gap:'14px',height:'100%'}}>
      ...
      {isLoggedIn?<HeaderUserTools/>:<>
        ...
        <button className="iconBtn" aria-label="Language" style={{width:'32px',height:'40px',padding:0,display:'grid',placeItems:'center'}}><UiIcon name="globe" size={20}/></button>
      </>}
    </div>
  </div></header>
}
