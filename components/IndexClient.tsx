...
export default function IndexClient(){
 ...
 return <main className={s.page}>
  ...
  <section className={s.shell}><div className={s.toolbar}><div className={s.filters}>{['ALL','LOW','MEDIUM','HIGH'].map(x=><button className={risk===x?s.active:''} key={x} onClick={()=>setRisk(x)}>{x==='ALL'?'전체':x}</button>)}</div><span className={s.muted}>실시간 기준가 · 서버 평가손익</span></div>
