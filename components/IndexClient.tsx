...
export default function IndexClient(){
 ...
 const [risk,setRisk]=useState('ALL'),[modal,setModal]=useState<Product|null>(null),[amount,setAmount]=useState('1000'),[balance,setBalance]=useState<number|null>(null),[agree,setAgree]=useState(false),[msg,setMsg]=useState('');
 async function load(){
  ...
  if(user){const {data:b}=await supabase.from('demo_balances').select('available').eq('user_id',user.id).eq('asset','USDT').maybeSingle();setBalance(b?Number(b.available):0)}else setBalance(null);
 }
 useEffect(()=>{load()},[]);
 ...
 return <main className={s.page}>
  ...
  <section className={s.shell}><div className={s.toolbar}><div className={s.filters}>{['ALL','LOW','MEDIUM','HIGH'].map(x=><button className={risk===x?s.active:''} key={x} onClick={()=>setRisk(x)}>{x==='ALL'?'전체':x}</button>)}</div><span className={s.muted}>DEMO 운용 · REAL INDEX OFF</span></div>
   <div className={s.grid}>{filtered.map(p=>{const pf=currentPerf(p.id);const pa=productAssets(p.id);return <article className={s.card} key={p.id}><div className={s.cardTop}><div><h3>{p.name}</h3><span className={s.muted}>{p.code}</span></div><span className={s.risk}>{p.risk_level}</span></div><p className={s.desc}>{p.description}</p><div className={s.assets}>{pa.map(a=><span className={s.asset} key={a.asset_symbol}>{a.asset_symbol} {(Number(a.target_weight)*100).toFixed(0)}%</span>)}</div><div className={s.metrics}><div><span>현재 기준가</span><b>{pf?Number(pf.index_value).toFixed(2):'—'}</b></div><div><span>MDD</span><b className={pf&&Number(pf.drawdown)<0?s.down:''}>{pf?`${Number(pf.drawdown).toFixed(2)}%`:'—'}</b></div><div><span>투자자</span><b>{pf?.investor_count??0}</b></div></div><div className={s.cardFoot}><small>최소 {Number(p.min_investment).toLocaleString()} USDT · {p.rebalance_frequency}</small><button className={s.btn} onClick={()=>{setModal(p);setAmount(String(p.min_investment));setMsg('')}}>투자하기</button></div></article>})}</div>
  </section>
  {modal&&<div className={s.modalBackdrop} onMouseDown={()=>setModal(null)}><div className={s.modal} onMouseDown={e=>e.stopPropagation()}><h2>{modal.name}</h2><span className={s.muted}>BITMATE INDEX 투자 설정</span><label className={s.field}>투자금 (USDT)<input value={amount} onChange={e=>setAmount(e.target.value)} inputMode="decimal"/></label><div className={s.preset}>{[100,500,1000,3000,5000].map(x=><button key={x} onClick={()=>setAmount(String(x))}>{x.toLocaleString()}</button>)}</div><div className={s.preview}><div><span>사용 가능 DEMO 잔액</span><b>{balance==null?'로그인 필요':`${balance.toLocaleString()} USDT`}</b></div>{productAssets(modal.id).map(a=><div key={a.asset_symbol}><span>{a.asset_symbol} · {(Number(a.target_weight)*100).toFixed(0)}%</span><b>{((Number(amount)||0)*Number(a.target_weight)).toFixed(2)} USDT</b></div>)}</div><label className={s.check}><input type="checkbox" checked={agree} onChange={e=>setAgree(e.target.checked)}/><span>구성자산 가격 변동에 따라 원금 손실이 발생할 수 있고, 분산투자가 손실 가능성을 제거하지 않는다는 점을 확인했습니다.</span></label>{msg&&<div className={s.warning}>{msg}</div>}<div className={s.modalActions}><button className={s.ghost} onClick={()=>setModal(null)}>취소</button><button className={s.btn} onClick={invest}>투자 시작</button></div></div></div>}
 </main>;
}
