...
export default function CfdTimedTradingClient(){
  ...
  async function load(){
    const [{data:{user}},{data:p}]=await Promise.all([
      ...
      supabase.from('cfd_products').select('id,symbol,display_name,category,current_price,bid,ask,min_order,max_order,last_price_at').eq('status','ACTIVE').eq('trading_enabled',true).order('sort_order')
    ]);
  ...
  const current=Number(selected?.current_price||0);
  const bid=Number(selected?.bid||selected?.current_price||0);
  const ask=Number(selected?.ask||selected?.current_price||0);
  ...
  async function startTrade(){
    ...
    try{
      ...
      const {error}=await supabase.rpc('start_cfd_timed_trade',{
        p_symbol:selected.symbol,
  ...
  return <main className={s.page}>
    ...
    <section className={s.marketBar}>
      <div className={s.symbolBox}><select value={selected?.id||''} onChange={e=>setSelected(products.find(x=>x.id===e.target.value)||null)}>{products.map(p=><option key={p.id} value={p.id}>{p.display_name}</option>)}</select><span>{selected?.symbol||'CFD'}</span></div>
      <div className={s.priceBox}><strong>{fmt(current)}</strong><span>현재가</span></div>
      <div className={s.metric}><span>Bid</span><b>{fmt(bid)}</b></div>
      <div className={s.metric}><span>Ask</span><b>{fmt(ask)}</b></div>
      <div className={s.metric}><span>Spread</span><b>{spread?fmt(spread,6):'—'}</b></div>
      <div className={s.metric}><span>거래시간</span><b>3분 / 5분</b></div>
    ...
    <section className={s.terminalGrid}>
      ...
      <section className={`${s.panel} ${s.orderPanel}`}>
        ...
        <div className={s.summary}>
          <div><span>서버 시작가</span><b>주문 시 확정</b></div>
          <div><span>예상 WIN 지급</span><b>{fmt(expectedPayout,0)}</b></div>
          <div><span>DRAW 지급</span><b>{fmt(amountNum,0)}</b></div>
          <div><span>LOSS 지급</span><b>0</b></div>
        </div>
        ...
        {activeTrade&&<div className={s.activeTrade}>
          <div className={s.activeTradeTop}><strong>{activeTrade.symbol} · {activeTrade.direction}</strong><span className={s.countdown}>{mm}:{ss}</span></div>
          <div className={s.tradeMeta}>
            <div><span>시작가</span><b>{fmt(activeTrade.start_price,8)}</b></div><div><span>주문금액</span><b>{fmt(activeTrade.amount)}</b></div>
            <div><span>거래시간</span><b>{activeTrade.duration_minutes}분</b></div><div><span>만료</span><b>{new Date(activeTrade.expires_at).toLocaleTimeString('ko-KR',{hour12:false})}</b></div>
          </div>
        </div>}
      </section>
    </section>
    ...
    <section className={s.accountStrip}>
      <div><span>Wallet</span><b>{fmt(summary?.wallet_balance)}</b></div>
      <div><span>Available</span><b>{fmt(summary?.available_balance)}</b></div>
      <div><span>Trade Hold</span><b>{fmt(summary?.trade_hold_balance)}</b></div>
      <div><span>누적 실현손익</span><b className={(summary?.realized_pnl||0)>=0?s.win:s.loss}>{fmt(summary?.realized_pnl)}</b></div>
    </section>
    ...
    <div className={s.ticker}><span>● 서버 자동 정산 활성</span>{products.slice(0,6).map(p=><span key={p.id}><b>{p.display_name}</b> {fmt(p.current_price)}</span>)}</div>
  </main>;
}
