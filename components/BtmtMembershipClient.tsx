...
export default function BtmtMembershipClient(){
 ...
 return <main className={s.page}>
   <section className={s.hero}><div className={s.shell}><div className={s.heroGrid}><div><span className={s.eyebrow}>BTMT PREMIUM MEMBERSHIP</span><h1>BTMT를 스테이킹하고<br/>거래금액 리워드를 적립하세요.</h1><p>멤버십 등급에 따라 거래금액의 <b>0.2%부터 최대 1.0%</b>가 USDT 기준 리워드로 적립됩니다. 수수료 할인 대신 실제 거래금액 기준 적립 구조로 운영됩니다.</p><div className={s.heroMeta}><span>BTMT 기준가 <b>1 BTMT = {fmt(settings.btmt_price_usdt,2)} USDT</b></span><span>로그인 계정 전용</span><span>서버 거래 이벤트 기준 적립</span></div></div><div className={s.accountCard}><small>MY MEMBERSHIP</small>{activeTier?<><strong>{activeTier.name}</strong><p>{fmt(membership!.staked_btmt,0)} BTMT · {(membership!.reward_rate*100).toFixed(1)}% 적립</p><div className={s.rewardBox}><span>누적 적립금</span><b>{fmt(balance,4)} USDT</b></div></>:<><strong>Membership Ready</strong><p>{userEmail}</p><div className={s.rewardBox}><span>누적 적립금</span><b>{fmt(balance,4)} USDT</b></div></>}</div></div></div></section>
